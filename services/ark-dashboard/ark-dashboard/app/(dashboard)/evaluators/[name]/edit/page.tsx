...
function EvaluatorEditContent({ namespace, evaluatorName }: EvaluatorEditContentProps) {
  const router = useRouter()
  const [evaluator, setEvaluator] = useState<EvaluatorDetailResponse | null>(null)
  ...
  useEffect(() => {
    const loadEvaluator = async () => {
      try {
        const data = await evaluatorsService.getDetailsByName(namespace, evaluatorName)
        setEvaluator(data)
      } catch (error) {
        toast({
          variant: "destructive",
