...
function QueryDetailContent() {
  ...
  useEffect(() => {
    const selectedTools =
      query?.targets?.filter((t) => t.type === "tool") || [];
    ...
    <>
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4">
        ...
        <div className="ml-auto flex gap-2">
          {!isNew && (
            <QueryEvaluationActions queryName={queryId} namespace={namespace} />
          )}
          {isNew && (
            <>
              <Button
              ...
              <table className="w-full">
                <tbody>
                  ...
                  <tr className="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <td className="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                      {query.evaluators?.length
                        ? `${query.evaluators.length} evaluator(s)`
                        : "—"}
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr className="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <td className="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                      {query.status?.tokenUsage
                        ? `${query.status.tokenUsage.promptTokens || 0} / ${
                            query.status.tokenUsage.completionTokens || 0
                          }`
                        : "—"}
                    </td>
                  </tr>
                  ...
                  >
                    ...
                    <div
                      className={
                        ...
                        query.targets?.filter((t) => t.type === "tool")
                          .length === 1
                          ? "p-3 border-r border-gray-200 dark:border-gray-700"
                          : ""
                      }
                    >
                      <Textarea
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {query.status.responses.map((response, index) => (
                            ...
                            >
                              {response.target
                                ? `${response.target.type}:${response.target.name}`
                                : `Response ${index + 1}`}
                            </TabsTrigger>
                          ))}
                        </TabsList>
